...
void procesarArchivoCiudades(string nombreArchivo){
	...
	if(!archivo.is_open()){
		cout << "No se puede abrir el archivo de ciudades.\n";
		return;
	}
	...
	while(getline(archivo, ciudad)){
		codigosCiudades.insert(pair<string, int>(ciudad, numVertices++));
...
void procesarSolicitudVuelo(){
	...
	if(codigosCiudades.count(ciudadOrigen) == 0){
		cout << "Lo siento, no volamos desde ese ciudad." << endl;
		return;
	}
